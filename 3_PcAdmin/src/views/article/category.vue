  ...
  <div>
    <sticky :className="'sub-navbar'">
      <div class="filter-container">
        ...
        <el-button type="primary" size="mini" v-if="checkPermission('articlecategory', 'btnAdd')" icon="el-icon-plus" @click="onBtnClicked('btnAdd')">新增</el-button>
        <el-button type="danger" size="mini" v-if="checkPermission('articlecategory', 'btnDel')" icon="el-icon-delete" @click="onBtnClicked('btnDel')">删除</el-button>
      </div>
    </sticky>
    <div class="app-container">
      <div class="bg-white">
        <el-table ref="mainTable" :key="tableKey" :data="list" v-loading="listLoading" border fit highlight-current-row style="width: 100%" @row-click="rowClick" @selection-change="handleSelectionChange">
          ...
          <el-table-column width="120" label="图片" show-overflow-tooltip>
            <template slot-scope="scope">
              <el-image style="width:50px;height:50px" :src="scope.row.urlIcon"></el-image>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" align="center" label="操作" width="130" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              <el-button type="text" v-if="checkPermission('articlecategory', 'btnEdit')" @click="openEdit(scope.row)">编辑</el-button>
              <el-button type="text" v-if="checkPermission('articlecategory', 'btnDel')" @click="handleDelete([scope.row])">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
      ...
      var addRouter = [
        {
          ...
          children: [
            {
              path: 'editcategory',
              component: () => import('@/views/article/editcategory'),
    ...
    loadList() {
      ...
      articlecategory.load(this.listQuery).then((res) => {
    ...
    openEdit(row) {
      ...
      this.$router.push({ name: 'articleEditCategory', params: { id: id } })
    },
    handleDelete(rows) {
      // 多行删除
      articlecategory.del(rows.map((u) => u.id)).then(() => {
        this.$notify({
          title: "成功",
          message: "删除成功",
          type: "success",
