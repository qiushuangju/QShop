...
namespace Qs.Repository
{
    public class UnitWork<TDbContext> : IUnitWork<TDbContext> where TDbContext : DbContext
    {
        ...
        public IQueryable<T> Find<T>(Expression<Func<T, bool>> exp = null, string orderby = "", int pageindex = 1, int pagesize = 10) where T : class
        {
        ...
        /// <returns></returns>
        public int Sum<T>(Expression<Func<T, bool>> where, Func<T, int> selector) where T : class
        {
            int sum = 0;
            try
            {
                ...
                sum = _context.Set<T>().Where(where).AsNoTracking().Sum(selector);
            }
            catch (Exception e)
            {
                sum = 0;
        ...
        /// <param name="selector"></param>
        /// <returns></returns>
        public decimal Sum<T>(Expression<Func<T, bool>> where, Func<T, decimal> selector) where T : class
        {
            decimal sum = 0;
            try
            {
        ...
        /// </summary>
        public void BatchAdd<T>(List<T> entities) where T : BaseEntity
        {
        ...
        public void AddOrUpdate<T>(T entity) where T : StringEntity
        {
            T modelNew = null;
            if (!entity.KeyIsNull())
            {
                modelNew = FirstOrDefault<T>(p => p.Id == entity.Id);
            }
            if (modelNew == null)
            {
                Add(entity);
        ...
        public void Save()
        {
            try
            {
                ...
                foreach (var item in entities)
                {//解决UnitWork.Save后查询还是报cannot be tracked because another instance with the key错误
                    _context.Entry(item).State = EntityState.Detached;
                    _context.Entry(entity: item).State = EntityState.Detached;
                }
            }
