...
namespace Qs.Comm.Database
{
    ...
    public static class DbDataConvertExtensions
    {
        ...
        public static object ToList(this DataTable dataTable, Type returnType)
        {
            ...
            // 获取类型真实返回类型
            var underlyingType = isGenericType ? returnType.GenericTypeArguments.FirstOrDefault() : returnType;
        ...
        {
            // 获取元组最底层类型
            var underlyingTypes = tupleType.GetGenericArguments().Select(u => u.IsGenericType ? u.GetGenericArguments().FirstOrDefault() : u);
