...
namespace Qs.App
{
    ...
    public class DbExtension
    {
        ...
        public DbExtension(IOptions<AppSetting> appConfiguration, QsDBContext QsDBContext, IHttpContextAccessor httpContextAccessor)
        {
            _appConfiguration = appConfiguration;
            _httpContextAccessor = httpContextAccessor;
            _contexts.Add(QsDBContext);  //如果有多个DBContext，可以按QsDBContext同样的方式添加到_contexts中
        }
        ...
        public List<KeyDescription> GetProperties(string moduleName)
        {
            ...
            const string domain = "qs.repository.domain.";
            IEntityType entity = null; 
            _contexts.ForEach(p =>
            {
                var names = p.Model.GetEntityTypes().Select(p => p.Name).ToList();
                entity = p.Model.GetEntityTypes()
                    .FirstOrDefault(u => u.Name.ToLower().Contains(domain + moduleName.ToLower()) );
                if (entity == null)
                {
                    entity = p.Model.GetEntityTypes()
                        .FirstOrDefault(u => u.Name.ToLower().Contains($"qs.repository.response.{moduleName.ToLower()}"));
                }
                if (entity == null)
                {
                    var xx = p.Model.GetEntityTypes().Select(p => p.Name.ToLower()).ToList();
                    entity = p.Model.GetEntityTypes()
                        .FirstOrDefault(u => u.Name.ToLower().Contains($"{moduleName.ToLower()}"));
                }
            });
            if (entity == null)
            {
